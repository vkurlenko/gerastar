...
<form method="post" enctype="multipart/form-data" action=""  name="form1" id="form1">
	<table>
		...
		<tr>
			<td>Название (eng):</td>
			<td><input type="text" name="p_title_en" size=70 value="<?=htmlspecialchars($razdel[0]['p_title_en']);?>"></td>
		</tr>
		<?
		}?>
		<tr>
			<td>URL ( только латиница, цифры, - и _ ):</td>
			<td><!--<strong><a href="http://<?=$_SERVER['HTTP_HOST']."/".htmlspecialchars($razdel[0]['p_url'])."/";?>" target="_blank">http://<?=$_SERVER['HTTP_HOST']."/".htmlspecialchars($razdel[0]['p_url'])."/";?></a></strong><br>-->
			<input type="text" name="p_url" size=70 value="<?=htmlspecialchars($razdel[0]['p_url']);?>"></td>
		</tr>
		<tr>
		...
		<tr>
			<td>Фоновое изображение (для текстовой страницы)</td>
			<td><select name="p_img" >
			<?
			$sql_2 = "select * from `".$_VARS['tbl_photo_name'].$_VARS['env']['photo_alb_background']."` order by `id` desc ";
			echo $sql_2;
			...
			<span style="font-size:10px;">(название картинки из фотобанка "<a href="/<?=$_VARS['cms_dir'];?>/workplace.php?page=photo&zhanr=<?=$_VARS['env']['photo_alb_background']?>" target="_self">Фоновые изображения</a>")</span>
			<?
				$img = new Image();
	...
	if($_VARS['multi_lang'] == true)
	{
		...
		<?
		$editor_text_edit = $razdel[0]['p_content_en'];
		$editor_text_name = 'p_content_en';
		$editor_height = 400;
		include "../".$_VARS['cms_modules']."/common/editor/fck_editor.php";		
		?>
		</fieldset>
	...
	?>
	<fieldset>
	<legend><strong>Нижнее меню</strong></legend>	
	<?
	...
	<table width="100%">
		...
		<tr>
			<td>Title (eng):</td>
			<td><input type="text" name="p_meta_title_en" size=70 value="<?=htmlspecialchars($razdel[0]['p_meta_title_en']);?>"></td>
		</tr>
		<tr>
			<td>Keywords (eng):</td>
			<td><input type="text" name="p_meta_kwd_en" size=70 value="<?=htmlspecialchars($razdel[0]['p_meta_kwd_en']);?>"></td>
		</tr>
		<tr>
			<td>Description (eng):</td>
			<td><input type="text" name="p_meta_dscr_en" size=70 value="<?=htmlspecialchars($razdel[0]['p_meta_dscr_en']);?>"></td>
		</tr>
		<?
		}
		?>
    ...
    <input type="submit" value='Сохранить' class="bigSubmit" onClick="document.form1.action='index.php'; document.form1.target='_self';MM_openBrWindow('/<?=$_VARS['cms_dir'];?>/log_write.php?text=<? echo "Изменен раздел "; ?>'+document.getElementById('p_title').value,'logwin','width=100,height=100');">
