...
class Msg_Debug
{
	public $msg_type_show = "all";

	public function msg_print($msg_type, $msg_text)
	{
...
class DB_Table
{
	public $tableName;			// имя таблицы	(string)
	public $tableFields;		// поля таблицы (array)
	public $tableData; 			// данные (array)
	public $tableWhere; 		// данные условия where (array)
	public $tableOrderField; 	// поле, служащее для сортировки данных таблицы
	public $tableFieldJump; 	// 
	public $tableJumpCount; 	// быстрый сдвиг на Х позиций
	public $debugMode = false; 	// флаг вывода отладочных сообщений
	public $createTestRecord = false;
	public $parent_id;
	...
	public function reOrderItem()
	{
		...
		/*$sql = "SELECT * FROM `".$this->tableName."` 
				...
				AND ".$this->tableWhere['parent_id']." = ".$row[$this -> tableWhere['parent_id']].") 
		...
		if(isset($this->tableWhere['parent_id']))
			$sql .= " AND ".$this->tableWhere['parent_id']." = ".$row[$this -> tableWhere['parent_id']]." ";
		
		$sql .= ") ORDER BY ".$this->tableOrderField." ".$this -> tableWhere['dir']." 
		LIMIT 1";
		//echo $sql;
		$res = mysql_query($sql);
	...
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	public function reOrderItemJump ()
	{
		// определим знак
		if($this -> tableWhere['dir'] == "asc") $arrow = ">";
		elseif($this -> tableWhere['dir'] == "desc") $arrow = "<";
	...
	/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/	
	public function debugMessage($msgString, $result)
	{
