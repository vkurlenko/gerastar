	...
	$res = mysql_query($sql);
		...
		while($row = mysql_fetch_array($res))
		{
			?><tr>
				...
				<td><a href="?page=<?=$_MODULE_PARAM['name']?>&editItem&id=<?=$row['id'];?>"><?=$row['elem_value'];?></a></td>
				<td><?=$row['elem_key'];?></td>	
				<td align="center"><?=iconChkBox($_MODULE_PARAM["tableName"], 'elem_show', $row['elem_show'], $row['id'])?></td>
				<td><a href="?page=<?=$_MODULE_PARAM['name']?>&editItem&id=<?=$row['id'];?>"><img src='<?=$_ICON["edit"]?>'></a></td>
				<td><a style='color:red' href="javascript:if (confirm('Удалить элемент?')){document.location='?page=<?=$_MODULE_PARAM['name']?>&delItem&id=<?=$row['id'];?>'}"><img src='<?=$_ICON["del"]?>'></a></td>
			</tr>
			<?
		}
		?>
