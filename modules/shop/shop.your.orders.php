...
{
	$sql = "select * from `$table_name` where orderUser = ".$_GET['param2']." and id = ".$_GET['param3'];
	$res = mysql_query($sql);	
	$row = mysql_fetch_array($res);
...
// выводим список всех заказов
	$sql = "select * from `$table_name` where orderUser = ".$_GET['param2']." order by orderDate desc";
	$res = mysql_query($sql);	
	?>
	<div class="list">
		<table width="100%" cellpadding="10" cellspacing="1">
			...
			while($row = mysql_fetch_array($res))
			{
				...
				<tr <?=$bg;?>>
					<td><?=$row['id']?></td>
					<td><a href="/your_orders/<?=$_GET['param2'];?>/<?=$row['id']?>/"><?=$row['orderDate'];?></a></td>
					<td><?=format_price($arrOrder['sum']);?></td>
					<td><?=$_VARS['order_status'][$row['orderStatus']];?></td>
					<td><?
					if(file_exists($_SERVER['DOCUMENT_ROOT']."/files/".$row['id'].".xls") && $row['orderStatus'] == "accepted")
					{
						?>
